...
function LazySection({ children, estimatedHeight = 280, scrollY }: {
  ...
  return (
    <View onLayout={onLayout} style={{ height: estimatedHeight }} />
  );
}
    ...
    <View style={[styles.container, { paddingTop: insets.top + webTopInset }]}>
      ...
      >
        ...
        {sortedSections.map((s: any, idx: number) => {
            ...
            const est = s.type === 'banner_slider' ? 200
              : s.type === 'static_banner' ? 200
              : s.type === 'brands_strip' ? 80
              : s.type === 'product_slider' ? 320
              : s.type === 'collection_products' ? 380
              : s.type === 'featured_products' ? 380
              : s.type === 'multi_collection' ? 360
              : s.type === 'collection_showcase' ? 360
              : 280;

            return (
              <LazySection key={s.id} estimatedHeight={est} scrollY={scrollYRef}>
                {renderSection()}
              </LazySection>
            );
          })
