...
export async function advancedSearch(options: SearchOptions): Promise<SearchResult> {
  ...
  const hasDirectMatch = directMatchCond.length > 0
    ? directMatchCond.join(' AND ')
    : 'TRUE';
  ...
  const sortWhereClause = (useRelevanceWrapping && tsQuery)
    ? (whereClause ? `${whereClause} AND (${hasDirectMatch})` : `WHERE (${hasDirectMatch})`)
    : whereClause;
